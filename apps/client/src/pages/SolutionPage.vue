...
import SpreadsheetBuilder from '../components/SpreadsheetBuilder.vue';
...
// View mode: 'chat', 'viewer', 'spreadsheet', or 'tools'
type ViewMode = 'chat' | 'viewer' | 'spreadsheet' | 'tools';
const viewMode = ref<ViewMode>('chat');
const spreadsheetRef = ref<InstanceType<typeof SpreadsheetBuilder> | null>(null);
...
function handleFileUploaded(file: {name: string, path: string, absolutePath: string, timestamp: number}) {
  ...
  console.log('✅ SolutionPage: File added via ChatWindow:', file, 'Total files:', uploadedFiles.value.length);
}
...
async function handleLeftUpload(file: File) {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log('✅ SolutionPage: File added via left sidebar:', fileObj, 'Total files:', uploadedFiles.value.length);
    }
  } catch (err) {
...
<template>
  <div class="min-h-screen theme-bg flex flex-col transition-colors duration-300">
    ...
    <header class="gradient-header px-6 py-4 flex items-center justify-between shadow-lg">
      ...
      <div class="flex items-center gap-6">
        ...
        <button
          ...
          class="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-sm font-medium text-white/80 hover:text-white"
          title="Switch language"
        >
          <span :class="{ 'text-white font-bold': language === 'en', 'text-white/50': language !== 'en' }">EN</span>
          <span class="text-white/30">|</span>
          <span :class="{ 'text-white font-bold': language === 'nl', 'text-white/50': language !== 'nl' }">NL</span>
        </button>
    ...
    <main class="flex-1 flex overflow-hidden">
      ...
      <div class="flex-1 flex flex-col p-4 gap-4 overflow-hidden">
        ...
        <div class="flex-1 min-h-0 relative">
          ...
          <!-- Spreadsheet Builder -->
          <div v-show="viewMode === 'spreadsheet'" class="absolute inset-0">
            <SpreadsheetBuilder
              ref="spreadsheetRef"
        ...
        <!-- Chat Input (visible in chat/viewer/spreadsheet modes) -->
        <ChatWindow
          v-show="viewMode !== 'tools'"
          :key="chatKey"
          :events="events"
    ...
    >
      ...
      <button
        @click="goHome"
        class="absolute top-6 left-6 flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-slate-800 bg-white/80 hover:bg-white rounded-lg shadow-sm transition-colors"
      >
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="max-w-md w-full mx-4">
        ...
        <div class="text-center mb-8">
          <router-link to="/">
            <img :src="BimaiLogo" alt="BIM AI" class="h-20 mx-auto mb-4 cursor-pointer hover:opacity-80 transition-opacity" />
          </router-link>
        </div>
