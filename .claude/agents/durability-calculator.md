...
1. **Batch elements file**: e.g., `batch_1_elements.json` (workspace root)
2. **Output filename**: e.g., `batch_1_co2_report.json` (workspace root)
...
2. **Exact paths**: Read from workspace root, write to workspace root
...
Input file: batch_1_elements.json
Output file: batch_1_co2_report.json

Read batch_1_elements.json, load the durability database,
calculate CO2 for ALL elements, and write complete report
to batch_1_co2_report.json.
```
