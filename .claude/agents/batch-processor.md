...
3. **Output filename**: Where to save results in workspace root, e.g., `batch_1_elements.json`
...
### Step 4: Write Output
```
Write to: {output_filename}  # In workspace root, NOT in session context
...
2. **Use exact paths**: Read from session context, write to workspace root
...
Batch number: 1
Output file: batch_1_elements.json

Read batches.json, extract batches[0] (batch 1), process ALL elements,
and write complete classification to batch_1_elements.json.
```
