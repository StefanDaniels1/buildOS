...
Aggregates CO2 calculation data from multiple batches and creates professional PDF report
"""
...
try:
    ...
    os.system("pip install reportlab pillow -q")
    try:
        from reportlab.lib.pagesizes import A4
        from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
        from reportlab.lib.units import inch, cm
...
def aggregate_data(all_data):
    ...
    for batch_idx, batch in enumerate(all_data, 1):
        ...
        if 'skipped_elements' in batch:
            all_skipped.extend(batch['skipped_elements'])
    ...
    return summary, dict(by_category), all_detailed, all_skipped, batch_summaries

def generate_pdf_report(output_file, summary, by_category, batch_summaries, all_detailed, all_skipped):
    ...
    # Sort categories by CO2 impact (descending)
    ...
    # Detailed Results Summary (first 20 calculated elements)
    if all_detailed:
...
def main():
    ...
    summary, by_category, all_detailed, all_skipped, batch_summaries = aggregate_data(all_data)
    ...
    # Generate PDF
    print("Step 4: Generating professional PDF report...")
    if generate_pdf_report(output_file, summary, by_category, batch_summaries, all_detailed, all_skipped):
